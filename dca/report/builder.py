...
def build(
    ...
    *,
    ...
    D["roll"] = (
        {"dates": [d.strftime("%Y-%m") for d in rd["start"]], "port": [round(float(x), 4) for x in rd["irr"]]}
        if len(rd)
        else None
    )
    ...
    if len(cand) >= 2:
        ...
        sens = A.asset_start_sensitivity(
            p, starts, horizon_y=horizon, use_proxy=use_proxy, monthly=p.monthly_amount, buy_fee=p.buy_fee
        )
            ...
            "rows": [
                {
                    "name": x["name"],
                    "kind": x["kind"],
                    "v": [None if x[c] != x[c] else round(float(x[c]), 4) for c in vc],
                    "lo": round(float(x["lo"]), 4),
                    "hi": round(float(x["hi"]), 4),
                    "range": round(float(x["range"]), 4),
                }
    ...
    if not mx.empty:
        D["matrix"] = {
            "years": [int(y) for y in mx.index],
            "hor": [int(c) for c in mx.columns],
    ...
    pert = A.weight_perturbation(
        r, w, horizon_y=horizon, n=n_pert, monthly=p.monthly_amount, buy_fee=p.buy_fee
    )
    try:
        boot = A.block_bootstrap(
            blend, horizon_y=horizon, n_paths=n_boot, monthly=p.monthly_amount, buy_fee=p.buy_fee
        )
    except Exception:
        boot = None
    ...
    P_ = PCT
    parts = [
        f"<title>{esc(p.name)} · 定投回测报告</title>",
...
<tbody>{agg}</tbody></table></div></section>""")
    ...
    def _pert_row(x):
        ...
        return (
            f"<tr><td>{esc(str(x['level']))}</td><td class='n'>{rng_txt}</td>"
    ...
    mcr = "".join(
        f"<tr><td>{esc(t)}</td><td class='n em'>{P_(s.median())}</td><td class='n'>{P_(s.quantile(0.05))}</td>"
        f"<td class='n'>{P_(s.quantile(0.25))}</td><td class='n'>{P_(s.quantile(0.75))}</td>"
        f"<td class='n {'pos' if (s < 0).mean() < 0.02 else ''}'>{P_((s < 0).mean(), 2)}</td></tr>"
        for t, s in mc.items()
    )
    bootline = (
        f"{n_boot} 条重采样路径：中位 <span class='mono'>{P_(boot.median())}</span>，"
        f"5% 分位 <span class='mono'>{P_(boot.quantile(0.05))}</span>，"
