...
def _fx_series(pair: str, cache_dir: str) -> pd.Series:
    ...
    s = (
        pd.Series(pd.to_numeric(d[col], errors="coerce").values, index=pd.to_datetime(d["日期"]))
        .dropna()
        .sort_index()
        / 100.0
    )
...
class Portfolio:
    ...
    def fetch(
        self, force: bool = False, verbose: bool = True, use_proxy: bool = False
        ...
        for a in self.resolved(use_proxy):
            ...
            if verbose:
                ...
                print(
                    f"  {a.name:16s} {a.source:9s} {a.code:<28s} "
                    f"{len(df):6d} 行  {df['date'].min().date()} ~ {df['date'].max().date()}  {yrs:5.1f}y"
                )
        return out

    # ---------- 对齐 ----------
    def monthly(self, force: bool = False, verbose: bool = False, use_proxy: bool = False) -> pd.DataFrame:
    ...
    def blend(
        self, r: pd.DataFrame | None = None, weights: pd.Series | np.ndarray | None = None
    ) -> pd.Series:
        """把收益率矩阵按权重合成单一序列。"""
        if r is None:
            r = self.returns()
