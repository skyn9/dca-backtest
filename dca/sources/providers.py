...
class FundSource(Source):
    ...
    def _fetch(self, code: str) -> pd.DataFrame:
        ...
        d["date"] = (
            pd.to_datetime(d["x"], unit="ms", utc=True)
    ...
    def fees(self, code: str) -> dict:
        ...
        h = http_get(
            f"https://fundf10.eastmoney.com/jjfl_{code}.html", referer="https://fundf10.eastmoney.com/"
        )
...
class AkshareSource(Source):
    ...
    def _fetch(self, code: str) -> pd.DataFrame:
        ...
        pcol = next(
            (c for c in d.columns if str(c) in ("close", "收盘", "收盘价", "px", "最新价", "晚盘价")), None
        )
